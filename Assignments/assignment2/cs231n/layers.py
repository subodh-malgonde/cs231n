...
def affine_backward(dout, cache):
  ...
  #############################################################################
  N = x.shape[0]
  x_reshaped = np.reshape(x, (N, -1)) # reshape into dimensions NxD
  dx = dout.dot(w.T)
  dx = np.reshape(dx, x.shape) # reshape into (N, d1, ..., d_k)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #normaliza the incoming data
    x_norm = (x-sample_mean)/np.sqrt(sample_var + eps)
    out = gamma*x_norm + beta

    # re-compute the running_mean
    running_mean = momentum*running_mean + (1-momentum)*sample_mean

    # re-compute the running_var
    running_var = momentum*running_var + (1-momentum)*sample_var

    # store the running mean and var for later use
    bn_param['running_mean'] = running_mean
    bn_param['running_var'] = running_var

    cache = (x, sample_mean, sample_var, gamma, beta, x_norm, eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  # taken from https://github.com/ncchen55414/Winter-2016-CS231N/blob/master/Assignment2/cs231n/layers.py
  (x, sample_mean, sample_var, gamma, beta, x_norm, eps) = cache
  N = x.shape[0]
...
def batchnorm_backward_alt(dout, cache):
  ...
  #############################################################################
  pass # did not implement this one
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
    ...
    for k in xrange(F): # kth filter
      ...
      for ls in xrange(H_):
        ...
        for ws in xrange(W_):
          ...
          #convolution window
          window = x_padded[i,:,length:length+HH, width:width+WW]
          out[i,k,ls, ws] = np.sum(np.multiply(window,w[k,:])) + b[k]
    ...
    for k in xrange(F):  # kth filter
      ...
      for ls in xrange(H_):
        ...
        for ws in xrange(W_):
          ...
          db[k] += dout[i, k, ls, ws]
          dw[k,:] += window*dout[i, k, ls, ws]
          padded_dx[i, :, length:length + HH, width:width + WW] += w[k,:]*dout[i, k, ls, ws]
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in xrange(N): # for every image
    for j in xrange(C): # for every channel
      for ls in xrange(H_):
        length = stride*ls
        for ws in xrange(W_):
          width = stride*ws
          window = x[i,j,length:length+pool_height, width:width+pool_width]
          out[i,j,ls,ws] = np.max(window)
    ...
    for j in xrange(C):  # for every channel
      for ls in xrange(H_):
        ...
        for ws in xrange(W_):
          ...
          dx[i, j, length:length + pool_height, width:width + pool_width] += (window == max_value)*dout[i, j, ls, ws]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
